...
class GridWorld:
    def __init__(self, gamma = 1.0, theta = 0.5, gridSize=4, rewardSize=-1):
        ...
        self.transitions = pd.read_csv("gw_stochastic.txt", 
                                       header = None, 
                                       sep = "\t",
                                       names=['current_state', 'action', 'next_state', 'reward', 'next_states', 'probabilities']).values
    ...
    def state_transition(self, state, action):
        ...
        for tr in self.transitions:
            if tr[0] == state and tr[1] == action:
                next_states   = np.array(tr[4].split(','), dtype='int')
                probabilities = np.array(tr[5].split(','), dtype='float64')
                reward        = tr[3]
